    ...
    public static void Prefix(CharacterOption __instance) {
        if (__instance.hb.justHovered) {
            if (__instance.locked == false) {
                ...
                String flavorText = (String) ReflectionHacks.getPrivate(__instance, CharacterOption.class,
                        "flavorText");
                int unlocksRemaining = (int) ReflectionHacks.getPrivate(__instance, CharacterOption.class,
                        "unlocksRemaining");
                int maxAscensionLevel = (int) ReflectionHacks.getPrivate(__instance, CharacterOption.class,
                        "maxAscensionLevel");
                Output.setupUIBufferMany(__instance.c.title, hp + " hp; " + gold + " gold",
                        TextParser.parse(flavorText), unlocksRemaining + " unlocks remaining",
