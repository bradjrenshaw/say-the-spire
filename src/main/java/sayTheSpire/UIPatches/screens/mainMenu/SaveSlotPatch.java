    ...
    public static class UupdatePatch {
        ...
        public static void Postfix(SaveSlot __instance) {
            if (__instance.slotHb.justHovered) {
                ...
                float completionPercentage = (float) ReflectionHacks.getPrivate(__instance, SaveSlot.class,
                        "completionPercentage");
                if (!__instance.emptySlot) {
                    Output.text(name, true);
                    Output.setupUIBufferMany(name, TextParser.parse(CharStat.formatHMSM(playtime) + TEXT[0]),
