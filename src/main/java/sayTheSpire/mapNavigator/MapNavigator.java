...
public class MapNavigator {
  ...
  public Boolean changePathChoice(int direction) {
    if (this.pathChoices.isEmpty()) return false;
    int target = this.pathChoice + direction;
    if (target < 0 || target >= this.pathChoices.size()) return false;
  ...
  public ArrayList<MapRoomNode> followLinearPath(Boolean forward) {
    ...
    if (this.viewingNode == null) return nodes;
    if (forward) {
      if (this.pathChoices.isEmpty()) return nodes;
  ...
  public Boolean moveToNextSegmentNode(Boolean forward) {
    ...
    if (this.viewingNode == null) return false;
    if (forward) {
  ...
  public MapRoomNode getPathChoice() {
    if (this.pathChoice < 0 || this.pathChoice >= this.pathChoices.size()) return null;
  ...
  public ArrayList<MapRoomNode> getPathChoices(MapRoomNode node) {
    ...
    for (MapRoomNode r : map.get(targetRow)) {
      ...
      if ((targetRow == 0 && r.y == 0)
          || (OutputUtils.playerIsFlying() && node.wingedIsConnectedTo(r))
  ...
  public void setViewingNode(MapRoomNode node) {
    if (this.viewingNode == node) return;
  ...
  public MapRoomNode popViewingPath() {
    if (viewingPath.isEmpty()) return null;
