...
public class SpeechManager {
   ...
   * @param interrupt Whether or not previous speech/braille/etc should be interrupted.
   ...
   *
   * @param handler The SpeechHandler to register
  ...
  public Boolean setup() {
    for (SpeechHandler handler : this.handlers) {
      if (!handler.detect()) continue;
      if (!handler.loadResources()) continue;
      if (!handler.load()) continue;
