  ...
  public String getPositionString() {
    ...
    switch (this.location) {
      ...
      case HAND_SELECT:
        ...
        CardGroup unselectedGroup =
            (CardGroup) ReflectionHacks.getPrivate(screen, HandCardSelectScreen.class, "hand");
        ...
        if (result == null) result = getListPositionString(unselected);
        return result;
        ...
        ArrayList<AbstractCard> group =
            (ArrayList<AbstractCard>)
                ReflectionHacks.getPrivate(
                    masterScreen, MasterDeckViewScreen.class, "tmpSortedDeck");
        if (group == null) {
          group = AbstractDungeon.player.masterDeck.group;
        }
        return this.getGridPositionString(group, cardsPerLine);
      case EXHAUST_PILE_VIEW:
        CardGroup exhaustGroup =
            (CardGroup)
                ReflectionHacks.getPrivate(
                    AbstractDungeon.exhaustPileViewScreen,
                    ExhaustPileViewScreen.class,
                    "exhaustPileCopy");
      ...
      case DRAW_PILE_VIEW:
        CardGroup drawGroup =
            (CardGroup)
                ReflectionHacks.getPrivate(
                    AbstractDungeon.gameDeckViewScreen, DrawPileViewScreen.class, "drawPileCopy");
        if (drawGroup == null) return null;
        cardsPerLine =
            (int)
                ReflectionHacks.getPrivate(
                    AbstractDungeon.gameDeckViewScreen, DrawPileViewScreen.class, "CARDS_PER_LINE");
        return this.getListPositionString(drawGroup.group);
      case DISCARD_PILE_VIEW:
        CardGroup discardGroup =
            (CardGroup)
                ReflectionHacks.getPrivate(
                    AbstractDungeon.discardPileViewScreen,
                    DiscardPileViewScreen.class,
                    "discardPileCopy");
